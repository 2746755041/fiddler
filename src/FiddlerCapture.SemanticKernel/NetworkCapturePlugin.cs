...
[Description("Operate the local HTTP or HTTPS capture engine and summarize captured traffic for debugging, packet capture, and report generation.")]
public sealed class NetworkCapturePlugin
{
    ...
    [Description("Start the local proxy capture engine when the user wants to begin live traffic capture and the proxy is not already running.")]
    ...
    [Description("Stop the local proxy capture engine when capture should end or when proxy side effects need to be reverted.")]
